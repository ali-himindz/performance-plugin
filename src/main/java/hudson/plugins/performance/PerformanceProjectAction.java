...
public final class PerformanceProjectAction implements Action {
	...
	protected static JFreeChart createRespondingTimeChart(
	...
	protected static JFreeChart createThroughputChart(
	...
	protected static JFreeChart createSummarizerTrend(
		...
		final JFreeChart chart = ChartFactory.createTimeSeriesChart(uri,
				"Time", "Response Time", dataset.get(0), true, true, false);
		chart.setBackgroundPaint(Color.white);
		 ...
		 * axis2.isAutoRange(); axis2.setLowerBound(0); plot.setRangeAxis(1,
		 * axis2); plot.setDataset(1, dataset.get(1));
		 * plot.mapDatasetToRangeAxis(1, 1);
		 * 
		 * final StandardXYItemRenderer renderer2 = new
		 * StandardXYItemRenderer(); renderer2.setSeriesPaint(0, Color.black);
	...
	private String getPerformanceReportNameFile(
			final PerformanceReportPosition performanceReportPosition) {
	...
	public void doRespondingTimeGraphPerTestCaseMode(StaplerRequest request,
		...
		for (AbstractBuild<?, ?> build : builds) {
			if (buildsLimits.in(nbBuildsToAnalyze)) {
				...
				List<UriReport> uriListOrdered = performanceReport
						.getUriListOrdered();
				for (UriReport uriReport : uriListOrdered) {
					dataSetBuilderAverage.add(uriReport.getAverage(),
		...
		ChartUtil.generateGraph(request, response,
				createRespondingTimeChart(dataSetBuilderAverage.build()), 600,
				200);
	...
	public void doThroughputGraph(final StaplerRequest request,
		...
		final List<? extends AbstractBuild<?, ?>> builds = getProject()
				.getBuilds();
		...
		for (final AbstractBuild<?, ?> build : builds) {
			if (buildsLimits.in(nbBuildsToAnalyze)) {
				...
				final PerformanceBuildAction performanceBuildAction = build
						.getAction(PerformanceBuildAction.class);
				if (performanceBuildAction == null) {
					continue;
				}
				...
				final ThroughputReport throughputReport = new ThroughputReport(
						performanceReport);
				final NumberOnlyBuildLabel label = new NumberOnlyBuildLabel(
						build);
				dataSetBuilder.add(throughputReport.get(),
						Messages.ProjectAction_RequestsPerSeconds(), label);
			}
			nbBuildsToAnalyze--;
		}
	...
	public void doSummarizerGraph(StaplerRequest request,
		...
		for (Iterator<?> iterator = builds.iterator(); iterator.hasNext();) {
			...
			if (buildsLimits.in(nbBuildsToAnalyze)) {
				...
				for (Map.Entry<String, UriReport> entry : performanceReport
						.getUriReportMap().entrySet()) {
					Long methodAvg = entry.getValue().getAverage();
					float methodErrors = entry.getValue().getSummarizerErrors();
					dataSetBuilderSummarizer.add(methodAvg, label,
							entry.getKey());
					dataSetBuilderSummarizerErrors.add(methodErrors, label,
							entry.getKey());
				}
			}
			nbBuildsToAnalyze--;
		...
		if (summarizerReportType != null) {
			ChartUtil.generateGraph(
					...
					createSummarizerChart(
							dataSetBuilderSummarizerErrors.build(), "%",
							Messages.ProjectAction_PercentageOfErrors()), 400,
					200);
		} else {
			ChartUtil.generateGraph(
					request,
					response,
					createSummarizerChart(dataSetBuilderSummarizer.build(),
							"ms", Messages.ProjectAction_RespondingTime()),
					400, 200);
		}
	}
			...
			if (graphConf.isDefaultDates()) {
				...
				try {
					firstDate = GraphConfigurationDetail
							.getGregorianCalendarFromString(graphConf
									.getFirstDayCount());
					...
					LOGGER.log(Level.SEVERE,
							"Error during the manage of the Calendar", e);
				}
				for (Iterator<?> iterator = builds.iterator(); iterator
						.hasNext();) {
					AbstractBuild<?, ?> currentBuild = (AbstractBuild<?, ?>) iterator
							.next();
					...
					if (lastBuild < 0
							&& lastDate.getTime().after(buildDate.getTime())) {
						lastBuild = var;
					}
					var--;
				}
	...
	public List<String> getPerformanceReportList() {
		...
		LOGGER.log(Level.ALL, "this.project=" + this.project);
		if (null == this.project) {
			return performanceReportList;
		}
		LOGGER.log(Level.ALL, "this.project.getSomeBuildWithWorkspace=" + this.project.getSomeBuildWithWorkspace());
//		this.project.getLastBuild().getWorkspace()
		LOGGER.log(Level.ALL, "this.project.getLastBuild().getWorkspace()="+this.project.getLastBuild().getWorkspace());
		LOGGER.log(Level.ALL, "this.project.getLastBuild().getArtifactsDir()="+this.project.getLastBuild().getArtifactsDir());
		LOGGER.log(Level.ALL, "this.project.getLastBuild().getRootDir()="+this.project.getLastBuild().getRootDir());
		if (null == this.project.getLastBuild().getRootDir()) {
			return performanceReportList;
		}

		File file = new File(this.project.getLastBuild().getRootDir(),
				PerformanceReportMap.getPerformanceReportDirRelativePath());
		
		LOGGER.log(Level.ALL, "file="+file);
		if (!file.isDirectory()) {
			return performanceReportList;
		}
		...
		for (File entry : file.listFiles()) {
			if (entry.isDirectory()) {
				for (File e : entry.listFiles()) {
					if (!e.getName().endsWith(".serialized")
							&& !e.getName().endsWith(".serialized-v2")) {
						this.performanceReportList.add(e.getName());
					}
				}
			} else {
				if (!entry.getName().endsWith(".serialized")
						&& !entry.getName().endsWith(".serialized-v2")) {
	...
	private DataSetBuilder<String, NumberOnlyBuildLabel> getTrendReportData(
	...
	public boolean ifSummarizerParserUsed(String filename) {
		...
		return this.getProject().getBuilds().getLastBuild()
				.getAction(PerformanceBuildAction.class)
				.getPerformanceReportMap().getPerformanceReport(filename)
				.ifSummarizerParserUsed(filename);
	}
